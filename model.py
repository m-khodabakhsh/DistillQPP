...
from auto import AutoModelForSequenceClassification
from PreTTR import PreTTR
...
Teacher_model_save_path = '/teacher/training_ms-marco_PreTTR-distilbert-base-uncased-e1'
Teacher_model = PreTTR.from_pretrained(Teacher_model_save_path)
class CrossEncoder():
    def __init__(self, model_name:str, num_labels:int = None, max_length:int = None, device:str = None, tokenizer_args:Dict = {},
                 default_activation_function = None, alpha:int = None):
        """
        ...
        """
        self.alpha = alpha
        self.config = AutoConfig.from_pretrained(model_name, cache_dir="/mnt/data/maryam/cache/")
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(model_name, config=self.config, cache_dir="/mnt/data/maryam/cache/" )
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, **tokenizer_args, cache_dir="/mnt/data/maryam/cache/" )
    ...
    def smart_batching_collate(self, batch):
        ...
        q_tokenized = self.tokenizer(texts[0], padding=True, truncation='longest_first', return_tensors="pt", max_length=30)
        p_tokenized = self.tokenizer(texts[1], padding=True, truncation='longest_first', return_tensors="pt", max_length=512-30)
        tokenized = self.tokenizer(*texts, padding=True, truncation='longest_first', return_tensors="pt", max_length=self.max_length)
        labels = torch.tensor(labels, dtype=torch.float if self.config.num_labels == 1 else torch.long).to(self._target_device)

        for name in tokenized:
            tokenized[name] = tokenized[name].to(self._target_device)
        for name in q_tokenized:
            q_tokenized[name] = q_tokenized[name].to(self._target_device)
            p_tokenized[name] = p_tokenized[name].to(self._target_device)

        return tokenized, labels, q_tokenized, p_tokenized
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for features, labels, q_features, p_features in tqdm(train_dataloader, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                if use_amp:
                    with autocast():
                    ...
                    Teacher_model.eval()
                    Teacher_model.to(self._target_device)
                    with torch.no_grad():
                        Teacher_CLSs = Teacher_model.forward(q_features,p_features)
                    model_predictions = self.model(**features, return_dict=True)
                    CLSs = model_predictions.hidden_states
                    logits = activation_fct(model_predictions.logits)
                    if self.config.num_labels == 1:
                        logits = logits.view(-1)
                    hidden_rep_loss = nn.CosineEmbeddingLoss()(CLSs, Teacher_CLSs, target=torch.ones(features['input_ids'].size(0)).to(self._target_device))
                    loss_value = loss_fct(logits, labels)

                    loss_value = alpha * hidden_rep_loss + loss_value
                    loss_value.backward()
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_grad_norm)
                    optimizer.step()
